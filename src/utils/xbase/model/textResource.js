...
export const PACKAGES = {
    ...
    MOBILE: [
        ...
        "apps.action.deletelastcategoriesvisited",
        "apps.categories.last",
        "apps.action.reportfraud",
        "apps.action.showsecurityhint",
        "apps.supiciouslisting",
        "apps.action.moredescription",
        "apps.action.lessdescription",
        "apps.detail.maps.title",
        "apps.detail.maps.openmaps",
        "apps.action.delete",
        "apps.action.done",
        "apps.yes",
        "apps.no",
        ...
        "apps.favourites.signintosync.message",
        'apps.gallery',
        'apps.home.more.lastviewed',
        'apps.home.more.favorites',
        "apps.needlogin.title",
        "apps.hinttitle",
        "apps.hintdescription",
        "apps.insertion.contact.countrycode", 
        "apps.advertphone",
        ...
        "apps.insertion.errordatastatefailadverthasnocontactaddress",
        "apps.insertion.errordatastatefailadvertcategoryismissing",
        "apps.insertion.errortitleempty",
        "apps.insertion.errordescriptionempty",
        "apps.insertion.erroraddressphone",
        'apps.statsviews',
        'apps.statsrequests',
        "apps.action.createlisting",
...
module.exports = {
    ...
    getAllTexts: function ({ lng }, callback) {
        let TAG = "textPack";
        // fLog(TAG, lng, "id: ", id)
        if (lng === 'en') {
            callback([]);
            ...
            let allPackage = [];
            // PACKAGES.map(item => {
            //     allPackage.push(...item)
            // })
            Object.keys(PACKAGES).forEach(function (key, index) {
                // key: the name of the object key
                // index: the ordinal position of the key within the object 
                allPackage.push(...PACKAGES[key])
            });
            fLog(TAG, 'allPackage = ', allPackage);
            let resultMap = new Map();
            let query = `SELECT XBResources.resourceKey AS key, text FROM XBResources, XBTextResources WHERE XBTextResources.language = '${lng}' AND XBResources.resourceId = XBTextResources.resourceId  AND XBResources.resourceKey IN  (${Array.from(allPackage, item => `'${item}'`)})`;
            fLog(TAG, '/textPack query', query)
            if (db) {
                fLog(TAG, '/db', db)
                db.transaction((tx) => {
                    fLog(TAG, 'tx = ', tx)
                    tx.executeSql(query, [], (tx, result) => {
                        ...
                        if (result.rows) {
                            // cache[lng][id] = result.rows;
                            for (let i = 0; i < result.rows.length; i++) {
                                let item = result.rows.item(i);
                                resultMap.set(item.key, item.text);
                            }
                            callback(resultMap)
                        }
                        else
