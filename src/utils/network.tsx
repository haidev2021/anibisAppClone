...
export const SERVER = 'http://192.168.0.105:3003';
...
export const ADVERT_DETAILS_BY_IDS_API = '/advert/detailsForApp';
export const ADVERT_SEARCH_API = '/advert/search';
export const ADVERT_MY_LISTING_IDS_API = '/advert/myAdvertIds';
...
export const IMAGE_UPLOAD_API = '/image/upload';
...
export function setAuthorizationTokenHeader(token: string) {
  // setHeader('Authorization', `Bearer ${token}`);
  POST_HEADERS.headers.Authorization = `Bearer ${token}`;
}

export function removeAuthorizationTokenHeader() {
  // setHeader('Authorization', `Bearer ${token}`);
  POST_HEADERS.headers.Authorization = '';
}
export function isAuthorized() {
  return POST_HEADERS.headers.Authorization;
}
